 ...
 *
 * The populator needs one name per entity to test it against a faction, and the first name of a
 * tag is the one the picker shows.
 */
[[nodiscard]] std::string_view first_name_of(std::uint32_t tag) noexcept {
    const auto names = names_of(tag);
    if (names.empty()) {
        return {};
    }
...
constexpr std::array<Faction, 6> kFactions{
    ...
    Faction{
        "Scorn",
        {"screeb", "chieftain", "raider", "ravager", "abomination", "stalker", "lurker", "wretch"}},
};
...
[[nodiscard]] bool champion_name(std::string_view name) noexcept {
    return std::any_of(
        kChampionMarkers.begin(), kChampionMarkers.end(), [name](std::string_view marker) {
...
[[nodiscard]] bool vehicle_name(std::string_view name) noexcept {
    return std::any_of(
        kVehicleMarkers.begin(), kVehicleMarkers.end(), [name](std::string_view marker) {
            return contains_insensitive(name, marker);
        });
}
...
[[nodiscard]] std::string_view leading_word(std::string_view name) noexcept {
    ...
    const std::string_view word = split == std::string_view::npos ? name : name.substr(0, split);
...
[[nodiscard]] bool is_combatant_name(std::string_view name) noexcept {
    ...
    return named && !(g_excludeVehicles && vehicle_name(name))
           && !(g_excludeChampions && champion_name(name)) && !(g_excludeBosses && boss_name(name));
}
...
void publish_population_source() noexcept {
    ...
    for (const Candidate& candidate : g_main.candidates) {
        const std::string_view name = first_name_of(candidate.tag);
        if (candidate.type != ObjectType::Biped || name.empty()) {
            continue;
        }
        if (!accepted_combatant(name)) {
            continue;
        }
        tags.push_back(candidate.tag);
    }
...
void draw_population() noexcept {
    ...
    if (g_populationSource == PopulationSource::map) {
        ...
        const std::size_t outcome = static_cast<std::size_t>(status.last) < kOutcomes.size()
    ...
    if (g_populationSource != PopulationSource::selected) {
        ...
        sourceChanged =
            ImGui::Checkbox("Exclude ships and vehicles", &g_excludeVehicles) || sourceChanged;
        ImGui::SameLine();
    ...
    changed = ImGui::SliderFloat("Forget distance", &g_population.forgetRadius, 20.0F, 1200.0F)
              || changed;
    changed =
        ImGui::Checkbox("Snap recorded points to ground", &g_population.snapToGround) || changed;
